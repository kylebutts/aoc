# Pt. 2 has a trick. Start at E and find shortest distance to 'a'
...
find_shortest_path = function(S_pos, E_pos)
...
println(find_shortest_path(S_pos, E_pos))

# There is a simpler way to do this but ¯\_(ツ)_/¯
costs = []
for i in 1:INPUT_SIZE[1]
  for j in 1:INPUT_SIZE[2]
    if input[i, j] == 'a'
      push!(costs, find_shortest_path([i, j], E_pos))
    end
  end
end

println(minimum(costs))
